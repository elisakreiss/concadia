...
class CaptionTSVDataset(Dataset):
    def __init__(self, yaml_file, label_cond, context_cond, randomized, tokenizer=None, add_od_labels=True,
            max_img_seq_length=50, max_seq_length=70, max_seq_a_length=40, max_context_length=50,
        ...
        self.context_cond = None if context_cond == 'none' else context_cond
        self.randomized_context = randomized == 'context'
        self.randomized_image = randomized == 'img'
        ...
        self.caption_file = find_file_path_in_yaml(self.cfg.get(self.label_cond), self.root)
        self.context_file = find_file_path_in_yaml(self.cfg.get(self.context_cond), self.root)

        assert op.isfile(self.feat_file)
        if add_od_labels: assert op.isfile(self.label_file)
        if self.context_cond: assert op.isfile(self.context_file)
        if self.randomized_context: assert self.context_cond
        ...
        self.captions: List[Dict[str, Union[str, int]]] = [] # keys: "image_id", "id", "caption"
        if self.caption_file and op.isfile(self.caption_file):
            with open(self.caption_file, 'r') as f:
                self.captions = json.load(f)
        if self.context_file and op.isfile(self.context_file):
            with open(self.context_file, 'r') as f:
                self.contexts = json.load(f) # list of dicts

        self.tokenizer = tokenizer
        self.tensorizer = CaptionTensorizer(self.tokenizer, max_img_seq_length,
                max_seq_length, max_seq_a_length, max_context_length, mask_prob, max_masked_tokens,
                is_train=is_train)
        self.add_od_labels = add_od_labels
        self.is_train = is_train
        self.kwargs = kwargs
        self.image_keys = self.prepare_image_keys() # key := image_id
        self.key2index = self.prepare_image_key_to_index()
        self.key2captions = self.prepare_image_key_to_captions()

        if self.randomized_context or self.randomized_image:
            randomized_indices = list(range(len(self.contexts))) if self.randomized_context \
                                    else list(range(len(self.key2index)))
            random.Random(42).shuffle(randomized_indices)
            self.randomized_indices = randomized_indices
    ...
    def get_image_index(self, idx):
        # more accurately, get_image_idx_from_caption_idx
        if self.is_train:
            img_cap_pair = self.captions[idx]
            img_key = img_cap_pair['image_id']
            img_idx = self.key2index[img_key]
            if self.randomized_image:
                img_idx = self.randomized_indices[img_idx]
            return img_idx
        return idx

    def get_image_key(self, idx):
        # more accurately, get_image_key_from_caption_idx
    ...
    def get_context(self, idx):
        ...
        if self.context_cond:
            if self.randomized_context and self.is_train:
                idx = self.randomized_indices[idx]
            img_context_pair = self.contexts[idx]
            context = img_context_pair['caption']
        return context
    ...
    def __getitem__(self, idx):
        ...
        example = self.tensorizer.tensorize_example(caption, features, text_b=od_labels, context=context)
...
class CaptionTSVDatasetWithConstraints(CaptionTSVDataset):
    ...
    def __init__(
        self, yaml_file, label_cond, context_cond, randomized,
        nms_threshold=0.85,
        max_given_constraints=3, **kwargs
    ):
        super().__init__(yaml_file, label_cond, context_cond, randomized, **kwargs)
...
class CaptionTensorizer(object):
    def __init__(self, tokenizer, max_img_seq_length=50, max_seq_length=70, 
            max_seq_a_length=40, max_context_length=50, mask_prob=0.15, max_masked_tokens=3,
            is_train=True):
        """Constructor.
        Args:
            tokenizer: tokenizer for text processing.
        ...
        self.max_text_seq_len = self.max_seq_len + self.max_context_len
        self.mask_prob = mask_prob
        self.max_masked_tokens = max_masked_tokens
        self._triangle_mask = torch.tril(torch.ones((self.max_seq_len, 
            self.max_seq_len), dtype=torch.long))

    def tensorize_example(self, text_a, img_feat, text_b=None, context=None,
        ...
        if text_b or context:
            ...
            if text_b:
                ...
                if len(tokens_b) > self.max_seq_len - len(tokens) - 1: # -1 accounts for the final sep token
                    tokens_b = tokens_b[: (self.max_seq_len - len(tokens) - 1)]
                tokens += tokens_b
                segment_ids += [sequence_b_segment_id] * len(tokens_b)
            if context:
                tokens_context = self.tokenizer.tokenize(context)[:self.max_context_len]
                tokens += tokens_context
                segment_ids += [sequence_b_segment_id] * len(tokens_context)
        ...
        seq_len = len(tokens)
        if self.is_train:
            masked_pos = torch.zeros(self.max_text_seq_len, dtype=torch.int)
            ...
            masked_pos = torch.ones(self.max_text_seq_len, dtype=torch.int)

        # pad on the right for image captioning
        padding_len = self.max_text_seq_len - seq_len
        ...
        max_len = self.max_text_seq_len + self.max_img_seq_len
        attention_mask = torch.zeros((max_len, max_len), dtype=torch.long)
        # C: caption, L: label, R: image region
        c_start, c_end = 0, seq_a_len
        l_start, l_end = self.max_seq_a_len, seq_len
        r_start, r_end = self.max_text_seq_len, self.max_text_seq_len + img_len
...
def build_dataset(yaml_file, label_cond, context_cond, randomized, tokenizer, args, is_train=True):
    if not op.isfile(yaml_file):
        yaml_file = op.join(args.data_dir, yaml_file)
        assert op.isfile(yaml_file)

    if is_train:
        return CaptionTSVDataset(yaml_file, label_cond, context_cond, randomized, tokenizer=tokenizer,
    ...
    return dataset_class(yaml_file, label_cond, context_cond, randomized, tokenizer=tokenizer,
            add_od_labels=args.add_od_labels, max_img_seq_length=args.max_img_seq_length,
...
def make_data_loader(args, yaml_file, label_cond, context_cond, randomized, tokenizer, is_distributed=True,
        is_train=True):
    dataset = build_dataset(yaml_file, label_cond, context_cond, randomized, tokenizer, args,
...
def save_checkpoint(model, tokenizer, args, epoch, ckpt_type=None, num_trial=10):
    checkpoint_subdir = 'checkpoint-{}'.format(epoch)
    if ckpt_type is not None:
        checkpoint_subdir = f'{ckpt_type.upper()}-{checkpoint_subdir}'
    checkpoint_dir = op.join(args.output_dir, checkpoint_subdir)
...
def compute_num_correct_in_topk_with_logits(logits, labels, k):
    _, ind = logits.topk(k, dim=-1, largest=True, sorted=True)
    correct_per_pred_per_seq = ind.eq(labels.unsqueeze(-1)) # (bsz * num_masked_per_seq, k)
    correct_per_seq = correct_per_pred_per_seq.sum(-1)
    return correct_per_seq


def train(args, train_dataloader, val_dataset, model, tokenizer, num_train_epochs, starting_epoch=0):
    if args.distributed:
    ...
    if args.max_steps > 0:
        ...
        num_train_epochs = args.max_steps // (len(train_dataloader) // \
    ...
    print(f"  Batch size per GPU = {args.per_gpu_train_batch_size}")
    print(f"  Total train batch size (w. parallel, & accumulation) = {args.per_gpu_train_batch_size * get_world_size() * args.gradient_accumulation_steps}")
    ...
    for epoch in range(starting_epoch, starting_epoch + num_train_epochs):
        for step, (img_keys, batch) in enumerate(train_dataloader):
            if args.debug and step == NUM_BATCHES_IN_DEBUG:
                break
            ...
            if not args.scst:
                ...
                batch_score = compute_num_correct_in_topk_with_logits(logits, masked_ids, k=5)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    print("Epoch: {}, global_step: {}, lr: {:.6f}, loss: {:.4f} ({:.4f}), " \
                        "score: {:.4f} ({:.4f})".format(epoch, global_step,
                        optimizer.param_groups[0]["lr"], latest_batch_loss, global_loss / global_step,
                        batch_acc, global_acc / global_step)
                    )

        if (args.save_epochs > 0 and epoch % args.save_epochs == 0) or \
                epoch == num_train_epochs - 1:
            checkpoint_type = "last" if epoch == num_train_epochs - 1 else None
            checkpoint_dir = save_checkpoint(model, tokenizer, args, epoch, ckpt_type=checkpoint_type)

        # end-of-epoch evaluation
        print(f"***** Perform evaluation at the end of epoch: {epoch} *****")
        evaluate_file = evaluate(args, val_dataset, model, tokenizer,
                op.join(args.output_dir, 'val_data'), epoch, 'val')
        with open(evaluate_file, 'r') as f:
            res = json.load(f)

        # check if there was an improvement
        is_best = res['CIDEr'] > best_score
        if is_best:
            best_score = res['CIDEr']
            best_epoch = epoch
        ...
        # add to eval log
        eval_log.append({'epoch': epoch, 'best_CIDEr': best_score, **res})
        with open(args.output_dir + '/eval_logs.json', 'w') as f:
            json.dump(eval_log, f, indent=4)

        # log stats at the end of each epoch; TODO: add eval stats
        print("* TRAIN - loss: {:.4f}, " \
            "score: {:.4f}".format(
        ...
        writer.add_scalar('train loss', global_loss / global_step, epoch)
        writer.add_scalar('train top5acc', global_acc / global_step, epoch)
        for metric, val in res.items():
            writer.add_scalar(f'val {metric}', val, epoch)

    # save best checkpoint
    checkpoint_dir = save_checkpoint(best_model, tokenizer, args, best_epoch, ckpt_type='best')
    return checkpoint_dir, best_model

def format_log_metrics(epoch, train_loss, train_top5acc, val_metrics):
    metrics = {'epoch': epoch}
    metrics['mlm_loss/train'] = train_loss
    metrics['mlm_top5acc/train'] = train_top5acc
    metrics.update({f'{key}/val_from_scratch': val for key, val in val_metrics.items() if key != 'SPICE'})
    return metrics
...
def scst_train_iter(args, train_dataloader, model, scst_criterion, 
    ...
    inputs = {'is_decode': True,
        ...
        'max_length': args.sc_max_gen_len,
...
def get_predict_file(output_dir, yaml_file, args, epoch):
    cc = ['pred']
    ...
    cc.append(data)
    cc.append(split)
    cc.append(epoch if isinstance(epoch, str) else f'{epoch:02d}')
...
def evaluate(args, val_dataloader, model, tokenizer, output_dir, epoch, split):
    predict_file = get_predict_file(output_dir,
            val_dataloader.dataset.yaml_file, args, epoch)
    test(args, val_dataloader, model, tokenizer, predict_file, split)
    print("Prediction results saved to: {}".format(predict_file))
    ...
    if is_main_process():
        ...
        if 'nocaps' not in data:
            result, pred_json_file = evaluate_on_coco_caption(predict_file, caption_file, outfile=evaluate_file)

            # consolidate preds and gt in one file
            pred_vs_gt_file = '.'.join(predict_file.split('.')[:-1])+'.debug.json'
            pred_vs_gt = []
            ...
            for gt, pred in zip(gts, preds):
                assert gt['image_id'] == pred['image_id']
                pred_vs_gt.append({'image_id': gt['image_id'], 'reference': gt['caption'], 'pred': pred['caption']})
            with open(pred_vs_gt_file, 'w') as f:
                json.dump(pred_vs_gt, f, indent=4)
...
def test(args, test_dataloader, model, tokenizer, predict_file, split):
    ...
    def gt_decode_eval():
        # ground-truth decoding to obtain loss, perplexity,acc
        global_step, global_loss, global_acc = 0, 0.0, 0.0
        with torch.no_grad():
            for step, (img_keys, batch) in tqdm(enumerate(test_dataloader)):
                ...
                inputs = {
                    ...
                    'token_type_ids': batch[2], 'img_feats': batch[3],
                    'masked_pos': batch[4], 'masked_ids': batch[5],
                    'is_training': False # TODO: verify this
                }
        ...
        avg_acc = global_acc / global_step
        avg_loss = global_loss / global_step
        perplexity = np.exp(avg_loss)
        print(f"* {split.upper()} - loss: {avg_loss:.4f}, " \
            f"score: {avg_acc:.4f}, perplexity: {perplexity:.4f})"
        )
        # TODO: log wandb
    ...
    def gen_rows():
        ...
        print("Inference model computing time: {} seconds per batch".format(time_meter / (step+1)))
...
def main():
    ...
    parser.add_argument('label_cond', type=str, choices=['description', 'caption'],
                        help='Output type.')
    parser.add_argument('context_cond', type=str, choices=['description', 'caption', 'context', 'none'],
    ...
    parser.add_argument("--data_dir", default='/home/ubuntu/s3-drive/data/vinvl_data', type=str, required=False,
                        help="The input data dir with all required files.")
    parser.add_argument("--model_name_or_path",
                        default='/home/ubuntu/s3-drive/models/vinvl_base_pretrained', type=str, required=False,
                        help="Path to pre-trained model or model type.")
    parser.add_argument("--output_dir", default='/home/ubuntu/s3-drive/concadia_oscar_runs/', type=str, required=False,
    ...
    parser.add_argument("--max_seq_length", default=82, type=int,
                        help="The maximum total input sequence length after tokenization. "
                             "Sequences longer than this will be truncated, "
                             "sequences shorter will be padded.")
    parser.add_argument("--max_seq_a_length", default=52, type=int,
                        help="The maximum sequence length for caption (including [CLS] and [SEP]).")
    parser.add_argument("--max_context_length", default=50, type=int,
                        help="The maximum sequence length for context (not including special tokens).")
    ...
    parser.add_argument("--num_train_epochs", default=35, type=int,
    ...
    parser.add_argument('--num_scst_epochs', type=int, default=5,
                        help="number of epochs for sc training")
    parser.add_argument("--scst_learning_rate", default=1e-6, type=float, help="The scst lr.")
    parser.add_argument('--sc_batch_size', type=int, default=2,
                        help="batch size for sc training")
    parser.add_argument('--sc_train_sample_n', type=int, default=1,
                        help="number of sampled captions for sc training")
    parser.add_argument('--sc_max_gen_len', type=int, default=20,
                        help="max gen length for sc training")
    ...
    parser.add_argument('--cider_cached_tokens', type=str, default='train-words.p',
                        help="path to cached cPickle file used to calculate CIDEr scores")
    
    # ===== for generation =====
    parser.add_argument("--eval_model_dir", type=str, default='', 
                        help="Model directory for evaluation.")
    parser.add_argument('--max_gen_length', type=int, default=52,
    ...
    if args.debug:
        ...
        args.num_scst_epochs = 2
        args.logging_steps = 1
        args.per_gpu_train_batch_size = 4
        args.output_dir = 'vinvl_debug_runs/'

    args.cider_cached_tokens = '-'.join([args.label_cond, args.cider_cached_tokens])

    # set up directories
    # create folder for current run
    now = datetime.now()
    run_type = f'{args.label_cond}_{args.context_cond}_{args.randomized}'
    folder_name = now.strftime("%Y%m%d_%H%M%S")
    args.output_dir = op.join(args.output_dir, run_type, folder_name)
    if args.seed == -1:
        args.seed = sum([int(t) for t in folder_name.split('_')])
    ...
    if args.do_train:
        ...
        print(f"Evaluate the following checkpoint: {checkpoint}")
        model = model_class.from_pretrained(checkpoint, config=config)

    model.to(args.device)
    print(f"Training/evaluation parameters {args}")
    if args.do_train:
        train_dataloader = make_data_loader(args, args.train_yaml, args.label_cond, args.context_cond, args.randomized, tokenizer,
            args.distributed, is_train=True)
        val_dataloader = make_data_loader(args, args.val_yaml, args.label_cond, args.context_cond, args.randomized, tokenizer,
                args.distributed, is_train=False)
        best_checkpoint, model = train(args, train_dataloader, val_dataloader, model, tokenizer, args.num_train_epochs)

        # test the last checkpoint after training
        if args.do_test:
            print("Evaluate on dataset: " + args.test_yaml)
            args.per_gpu_eval_batch_size = 16
            args.num_beams = 5
            test_dataloader = make_data_loader(args, args.test_yaml, args.label_cond, args.context_cond, args.randomized,
                tokenizer, args.distributed, is_train=False)
            evaluate(args, test_dataloader, model, tokenizer, op.join(args.output_dir, 'test_eval'), 'best', 'test')
    ...
    # inference and evaluation
        ...
        if not args.do_eval:
            predict_file = get_predict_file(checkpoint, test_dataloader.dataset.yaml_file, args, 'best')
            test(args, test_dataloader, model, tokenizer, predict_file, 'test')
            print("Prediction results saved to: {}".format(predict_file))
        else:
            evaluate_file = evaluate(args, test_dataloader, model, tokenizer,
                    checkpoint, 'best', 'test')
            print("Evaluation results saved to: {}".format(evaluate_file))
